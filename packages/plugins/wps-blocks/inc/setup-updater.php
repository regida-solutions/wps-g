...
function plugin_info( $res, string $action, object $args ) { //phpcs:ignore
	...
	$remote = get_transient( 'prefix_upgrade_' . WPS_BLOCKS_PLUGIN_SLUG );
	...
	if ( false === $remote ) {
		...
		if ( ! is_wp_error( $remote ) && isset( $remote['response']['code'] ) && 200 === $remote['response']['code'] && ! empty( $remote['body'] ) ) {
			set_transient( 'prefix_upgrade_' . WPS_BLOCKS_PLUGIN_SLUG, $remote, 21600 ); // 6 hours cache.
		}
	}
...
function plugin_push_update( $transient ) { //phpcs:ignore
	...
	if ( false === $remote ) {
		...
		$remote = wp_remote_get( WPS_BLOCKS_UPDATE_URL . '/' . WPS_BLOCKS_UPDATE_FOLDER . '/get-info.php?slug=' . WPS_BLOCKS_PLUGIN_SLUG . '&action=update',
			[
				'timeout' => 10,
				'headers' => [
					'Accept' => 'application/json',
...
function plugin_after_update( \WP_Upgrader $upgrader_object, array $options ):void {
	if ( 'update' === $options['action'] && 'plugin' === $options['type'] ) {
		...
		delete_transient( 'prefix_upgrade_' . WPS_BLOCKS_PLUGIN_SLUG );
	}
}
